...
class cets_blog_defaults 
{
	...
	function set_blog_defaults($blog_id, $user_id)
    {
		...
		$options = array(
			 ...
			 'blogname' => 'My Blog',
			 'blogname_flag' => 0,
			 'blogdescription' => sprintf(__('%s - Stuff you need to Code Better!'), $current_site->site_name ),
			 'comment_registration' => 0,
			 'gmt_offset' => date('Z') / 3600,
			 'timezone_string' => '',
			 'date_format' => __('F j, Y'),
		...
		if ( isset($options['permalink_structure']) || isset($options['category_base']) || isset($options['tag_base']) ) {
			...
			update_option("sidebars_widgets", array("primary-widget-area" => array("user_bio"), "secondary-widget-area" => array("archives", "links")));
   ...
   //Add the site-wide administrator menu  
	function add_siteadmin_page(){
       ...
       add_submenu_page('settings.php', 'New Blog Defaults', 'New Blog Defaults', 'unfiltered_html', 'cets_blog_defaults_management_page', array(&$this, 'cets_blog_defaults_management_page'));
