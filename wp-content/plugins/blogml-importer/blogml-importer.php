...
?>
	...
	function parse_blogml()
	{
		...
		// Get Categories
		$result = $this->xPath->match("/blog/categories/category/title/");
		$resultIds = $this->xPath->match("/blog/categories/category");
		$countResult = count($result);
		for ($i = 0; $i < $countResult; $i++)
		{
			
			$categoryId = $this->xPath->getAttributes($resultIds[$i], "id");
			$category = $this->xPath->getData($result[$i]);		
...
<p><?php _e('If a new user is created by WordPress, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)'); ?></p>
	...
	function process_posts() {
		...
		{
			$output_filename = '../permalinkmap.txt';
...
//				$csv_file_contents .= sprintf("%s %s\n", str_replace(',', ',,', $key), str_replace(',', ',,', $value));
			...
			$fhandle = fopen($output_filename, 'a');
	...
	function process_post($post) {
		...
		$blogml_post_approved      = $this->xPath->getAttributes($post, "approved");
		$post_status               = ($blogml_post_approved=='true') ? "publish" : "draft"; // hard-code to either publsished or draft (draft could be changed to "private")
		...
		for ($cat_index = 0; $cat_index < $cat_count; $cat_index++) {
			//echo $this->xPath->getAttributes($cat_results[$cat_index], 'ref');
			$categories2[$cat_index] = $this->xPath->getAttributes($cat_results[$cat_index], 'ref'); //$wpdb->escape($this->xPath->getAttributes($cat_results[$cat_index], 'ref'));
		}			
		...
		if ($post_id = post_exists($post_title, '', '')) {
			...
			if (count($categories2) > 0) {
				...
				foreach ($categories2 as $category) {					
					$cat_name = $this->categories[$category];										
					$cat_ID = (int) $wpdb->get_var("SELECT term_id FROM $wpdb->terms WHERE name = '$cat_name'");
					if ($cat_ID == 0) {
						echo 'Adding Category' . $cat_name;
